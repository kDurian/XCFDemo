...
typedef NS_ENUM(NSUInteger, FeedsKind) {
    FeedsKindRecipe = 1001,
    FeedsKindDish = 1005
};

static NSString * const kFeedsCommentStoryboardID = @"kFeedsCommentController";
static NSString * const kFeedsStoryboardName = @"Feeds";

static NSString * const kFeedsNotificationCellID = @"kFeedsNotificationCell";
...
@property(nonatomic, strong) NSMutableDictionary *k_Section_v_RowNum;
@property(nonatomic, strong) NSArray *latest_comments;
@property(nonatomic, assign) BOOL hasUnreadNotification;
@end
...
- (void)viewDidLoad {
    ...
    if (self.content.number == 0) {
        self.hasUnreadNotification = NO;
    }else {
        self.hasUnreadNotification = YES;
    }

    self.feeds = [NSMutableArray new];
    self.k_Section_v_RowNum = [NSMutableDictionary new];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    return self.hasUnreadNotification ? self.feeds.count + 1 : self.feeds.count;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (self.hasUnreadNotification) {
        if (section == 0) {
            return 1;
        }else {
            if (self.feeds.count > section - 1) {
                Feed *feed = self.feeds[section - 1];
                return [self numberOfRowsInSection:section withFeed:feed];
            }
        }
    }else {
...
- (NSInteger)numberOfRowsInSection:(NSInteger)section withFeed:(Feed *)feed{
    if (feed.kind == FeedsKindRecipe) {
        FeedRecipe *recipe = feed.recipe;
        if (recipe.desc.length == 0) {
            self.k_Section_v_RowNum[@(section).stringValue] = @3;
            return 3;
        }else {
            self.k_Section_v_RowNum[@(section).stringValue] = @4;
            return 4;
        }
        ...
        if (dish.desc.length == 0) {
            if (dish.digg_users.count == 0) {
                if (dish.latest_comments.count == 0) {
                    ...
                    self.k_Section_v_RowNum[@(section).stringValue] = @(3 + dish.latest_comments.count);
                    return 3 + dish.latest_comments.count;
                }else if (dish.latest_comments.count > 2) {
                    self.k_Section_v_RowNum[@(section).stringValue] = @6;
                    return 3 + 1 + 2;
                }
            }else {
                if (dish.latest_comments.count == 0) {
                    ...
                    self.k_Section_v_RowNum[@(section).stringValue] = @(4 + dish.latest_comments.count);
                    return 4 + dish.latest_comments.count;
                }else if (dish.latest_comments.count > 2) {
                    self.k_Section_v_RowNum[@(section).stringValue] = @7;
                    return 4 + 1 + 2;
                }
            }
        }else {
            if (dish.digg_users.count == 0) {
                ...
                if (dish.latest_comments.count == 0) {
                    self.k_Section_v_RowNum[@(section).stringValue] = @5;
                    return 5;
                }else if(dish.latest_comments.count <= 2) {
                    self.k_Section_v_RowNum[@(section).stringValue] = @(5 + dish.latest_comments.count);
                    return 5 + dish.latest_comments.count;
                }else if (dish.latest_comments.count > 2) {
                    self.k_Section_v_RowNum[@(section).stringValue] = @8;
                    return 5 + 1 + 2;
                }
            }
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.hasUnreadNotification) {
        if (indexPath.section == 0) {
            FeedsNotificationCell *cell = [tableView dequeueReusableCellWithIdentifier:kFeedsNotificationCellID forIndexPath:indexPath];
            cell.content = self.content;
            return cell;
        }else {
            if ((NSInteger)self.feeds.count > indexPath.section - 1) {
                Feed *feed = self.feeds[indexPath.section - 1];
                return [self tableView:tableView cellForRowAtIndexPath:indexPath withFeed:feed];
            }
        }
    }else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath withFeed:(Feed *)feed{
    switch (feed.kind) {
        case FeedsKindRecipe:{
            ...
            NSNumber *numberOfRowsInSection = self.k_Section_v_RowNum[@(indexPath.section).stringValue];
            NSInteger lastRowNum = numberOfRowsInSection.integerValue - 1;
        ...
        case FeedsKindDish:{
            ...
            NSInteger lastRowNum = numberOfRowsInSection.integerValue - 1;
            if (indexPath.row == lastRowNum) {
                self.latest_comments = dish.latest_comments;
...
- (UITableViewCell *)tableView:(UITableView *)tableView  cellForRowAtIndexPath:(NSIndexPath *)indexPath WithBaseNumber:(NSInteger)baseNumber andFeedRecipe:(FeedDish *)dish{
    ...
    NSAssert(count != 0, @"latest_commnets.count == 0");
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.hasUnreadNotification) {
        if (indexPath.section == 0) {
            return [tableView fd_heightForCellWithIdentifier:kFeedsNotificationCellID cacheByIndexPath:indexPath configuration:nil];
        }else {
            if (self.feeds.count > indexPath.section - 1) {
                Feed *feed = self.feeds[indexPath.section - 1];
                return [self tableView:tableView heightForRowAtIndexPath:indexPath withFeed:feed];
            }
        }
    }else {
        if (self.feeds.count > indexPath.section) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath withFeed:(Feed *)feed{
    switch (feed.kind) {
        case FeedsKindRecipe:{
            FeedRecipe *recipe = feed.recipe;
            if (indexPath.row == 0) {
...
- (void)commentButtonOfDishDidClicked{
    ...
    FeedsCommentController *controller = [storyboard instantiateViewControllerWithIdentifier:kFeedsCommentStoryboardID];
    controller.comments = self.latest_comments;
...
- (void)showMoreButtonOfDishDidClicked{
    ...
    UIAlertAction *shareAction = [UIAlertAction actionWithTitle:@"分享作品" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    }];
